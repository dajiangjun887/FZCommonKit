...
public extension UIControl {
    ...
    @objc func dx_HandleClick(events: UIControl.Event, click: ((UIControl) -> ())?){
        objc_setAssociatedObject(self, &UIControlActionHandlerKey, click, objc_AssociationPolicy.OBJC_ASSOCIATION_COPY_NONATOMIC)
        self.addTarget(self, action: #selector(dx_HandleClickEvent(_:)), for: events)
    }
    
    @objc private func dx_HandleClickEvent(_ sender: UIControl){
